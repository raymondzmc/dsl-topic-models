...
# Example: scripts/run_dsl.sh generative ERNIE-4.5-0.3B-PT 20_newsgroups 50 --temperature 2
cd "$(dirname "$0")/.." && source scripts/common.sh

if [ "$#" -lt 4 ]; then
  echo "Usage: $0 <model> <lm> <dataset> <K> [extra flags...]"
  echo "  model:   generative | generative_ecrtm | generative_fastopic | generative_etm | <baseline>"
  echo "  lm:      ${LMS[*]}"
